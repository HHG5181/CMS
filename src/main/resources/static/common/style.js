...
$(function(){
    ...
    $.msg = new function (that) {
        ...
        this.confirm = function (msg, ok, no) {
            var index = layer.confirm(msg, {title:false, btn: ['确认', '取消'],btnAlign: 'c'}, function () {
        ...
        this.auto = function(ret,time,callback){
    ...
    $.vali = function (form, callback, options) {
        return (new function (that) {
            ...
            this.hideError = function (ele) {
                ...
                $(ele).removeClass('error-text');
            };
            // 错误消息标签插入
            this.insertError = function (ele) {
                $(ele).addClass('error-text');
            };
            // 表单验证入口
    ...
    $.form = new function (that) {
        ...
        this.reInit = function ($dom) {
            ...
            $dom.find('input[data-date-input]').map(function () {
                console.log(this.getAttribute('data-range'))
                laydate.render({
                    type: this.getAttribute('data-date-input') || 'date',
                    range:this.getAttribute('data-range')!=null?'/':false, 
                    elem: this,
                    theme: '#23262E', 
        ...
        this.modal = function (url, data, title, callback, loading, tips,area) {
            this.load(url, data, 'GET', function (res, index) {
                ...
                index = layer.open({
                    type: 1, btn: false,anim:5,shade:0.3,skin: 'modal-class',area: area||"800px", content: res, title: title || '', success: function (dom, index) {
    ...
    $.fn.uploadOneImage = function () {
     ...
     $body.find('[data-custom-img]').map(function () {
        var file_key = $(this).attr('data-file') || 'image',that = this;
        window.upload.render({
            elem:this
            ,url: window.MODIAN + 'api/plugs/uploadImg'
            ,accept: 'images'
            ,field:'image'
            ...
            ,done: function(res){
                if(res.code === 1){
                    $.msg.success(res.msg)
                    $(that).css('backgroundImage','url('+res.data.url+')').trigger('change');
                    $("[name='"+file_key+"']").val(res.data.file_token)
                }else{
                    $.msg.error(res.msg)
                }
            }
    ...
    $body.on('click', '[data-select]', function () {
        ...
        var location = window.ROOT_URL + '/api.data/'+$(this).attr('data-select')+'.html?field=' + field;
    ...
    $body.on('click', '[data-upload]', function () {
        var file_key = $(this).attr('data-file-key') || 'image';
        var count = $(this).attr('data-upload') || 1;
        var fileId = $(this).attr('data-fileId') || '';
        var location = window.ROOT_URL + '/api.plugs/files.html?count='+count+'&file_key=' + file_key ;
        $.form.iframe(location, $(this).attr('data-title')||'图片库',['1030px','760px']);
    });
    /*! 注册 data-upload-mult 事件行为 */
    $body.on('click', '[data-upload-mult]', function () {
        var field = $(this).attr('data-upload-mult') || $(this).attr('data-field') || 'image';
        var location = window.ROOT_URL + '/api.plugs/files.html?field=' + field + '&multiple=true';
        return $.form.modal(location, [],  '图片库',null,null,null,['1030px','820px']);
    });
    ...
    $body.on('click', '[data-modal]', function () {
        ...
        var $this = $(this),data={};
        if($this.attr('data-rule') || $this.attr('data-value')){
            ...
            if (rule.length < 1) return require(['message'], function (_$) {
                _$.add('请选择需要的数据！','error', 1500);
            });
        ...
        data.open_type = 'modal'
        return $.form.modal($(this).attr('data-modal'), data, $(this).attr('data-title') || $(this).text() || '编辑',null,null,null,$(this).attr('data-area'));
    });
    /*! 注册 data-open 事件行为 */
    ...
    $body.on('click', '[data-address]', function () {
        ...
        var location = window.ROOT_URL + '/api.plugs/getpoint.html?field=' + field;
        $.form.iframe(location, '地址选择',['1000px','840px']);
    });
    ...
    $body.on('blur', '[data-action-blur]', function () {
        ...
        that.callback = function (ret) {
            if(parseInt(ret.code) === 1){
                if(ret.url!=='noback'){
                    return ret.url? (window.location.href = ret.url) : $.form.reload();
                }   
                return false;
            }else{
                return $this.css('border', (ret && ret.code) ? '1px solid #e6e6e6' : '1px solid red')
            }
    ...
    $body.on('click', '[data-icon]', function (field, location) {
        ...
        location = window.ROOT_URL + '/api.plugs/icon.html?field=' + field;
